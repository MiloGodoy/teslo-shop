...
export const paypalCheckPayment = async( paypalTransactionId: string ) => {
    ...
    // TODO: Realizar la actualización en base de datos
    try {
    ...
    return {
        ...
        return {
            ok: false,
            message: '500 - El pago no pudo realizarse'
        }
    }
}
...
const verifyPaypalPayment = async( paypalTransactionId: string, bearerToquen: string ): Promise<PayPalOrderStatusResponse | null> => {

    const paypalOrderUrl = `${ process.env.PAYPAL_ORDERS_URL }/${ paypalTransactionId }`;

    const myHeaders = new Headers();
    myHeaders.append("Authorization", `Bearer ${ bearerToquen }`);
