...
export const generatePaginationNumbers = ( currentPage: number, totalPages: number) => {
    ...
    // Si el número total de páginas es 7 o menos, se muestra todas las páginas sin puntos suspensivos
    if ( totalPages <= 7 ) {
        return Array.from({ length: totalPages }, (_, i) => i + 1); // [1,2,3,4,5,6,7]
    }

    // Si la página actual está entre las primeras 3 páginas
    // mostrar las primeras 3, puntos supensivos, y las últimas 2
    if ( currentPage <=3 ) {
